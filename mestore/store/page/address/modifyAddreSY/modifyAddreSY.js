...
Page({
  ...
  delete: function() {
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          wx.request({
            url: 'http://mestore.vicp.io/store/order/deleteAddress?index=' + that.data.index + "&id=" + that.data.id,
            success: function() {
              wx.showToast({
                title: '删除成功！',
              })
  ...
  formSubmit: function(e) {
    ...
    if (e.detail.value.name == "") {
      ...
      wx.request({
        url: 'http://mestore.vicp.io/store/order/modifyAddress?tell=' + e.detail.value.tell + "&door=" + e.detail.value.door + "&name=" + e.detail.value.name + "&area=" + e.detail.value.area + "&index=" + that.data.index + "&id=" + that.data.id,
        success: function() {
          wx.showToast({
            title: '保存成功！',
          })
